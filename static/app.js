...
function addLocalVideo() {
    Twilio.Video.createLocalVideoTrack().then(track => {
        let vid = document.getElementById('local').firstElementChild;
        let trackElement = track.attach();
        trackElement.addEventListener('click', () => { zoomTrack(trackElement); });
        trackElement.id = "host"
        vid.appendChild(trackElement);
    });
};
        ...
        fetch('/login', {
            ...
            if(username=='abhiraj'){
                const connectOptions = {
                    audio: navigator.mediaDevices.getUserMedia({ video: true })
                      .then(() => false)
                      .catch(() => true)
                };
